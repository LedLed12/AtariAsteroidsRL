...
class AtariAsteroidsImpl:
    ...
    def __init__(self, render_mode):
        ...
        self.bullets = []
        self.token = myenv.toke.generate_token()
    ...
    def do_action(self, action_index):
        ...
        if action_index == 1:  # Left Rotate
            ...
            self.spaceship.rotation_speed = 0
    ...
    def observe(self):
        ...
        :return:
        """
       # tok =  if self.token.position() is not None else (0, 0)
        print(self.token.position())
        obs = {"Agent": np.array(self.spaceship.position, dtype=int),
               #"Asteroid": np.array(len(self.asteroids),self.asteroids[0].position, dtype=int),
               #"Token": np.array(self.token.position(), dtype=int)
               }
        return obs
    ...
    def draw_Asteroids(self):
        self.update_Asteroid_position()
        for asteroid in self.asteroids:
            self.window.blit(asteroid.image, asteroid.position)


    def update_Asteroid_position(self):
        for asteroid in self.asteroids:
